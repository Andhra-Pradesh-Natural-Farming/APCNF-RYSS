...
async function submitCatchmentRequest() {
    ...
    try {
        // Construct Catchment Object for Local Persistence
        const catchmentObj = {
            id: 'ACM-' + Date.now() + '-' + Math.floor(Math.random() * 1000),
            catchment_id: 'ACM-' + Date.now(),
            user_id: aggregatorId,
            aggregator_id: aggregatorId, // Explicitly map for table display
            district_id: districtSelect.value,
            district_name: getSelectedText(districtSelect),
            mandal_id: mandalSelect.value,
            mandal_name: getSelectedText(mandalSelect),
            panchayat_id: panchayatSelect.value,
            panchayat_name: getSelectedText(panchayatSelect),
            village_id: villageSelect.value,
            village_name: getSelectedText(villageSelect),
            vo_id: getSelectedValue(voSelect) || '',
            vo_name: getSelectedText(voSelect) || 'NA',
            shg_id: getSelectedValue(shgSelect) || '',
            shg_name: getSelectedText(shgSelect) || 'NA',
            crop_id: cropSelect.value,
            crop_name: getSelectedText(cropSelect),
            crop_variety_id: varietySelect.value || 'NA',
            crop_variety_name: getSelectedText(varietySelect) || 'NA',
            season: seasonSelect.value,
            period: periodSelect.value,
            // Critical: Auto-Approve
            status: 1,
            approval_status: 'Approved',
            created_at: new Date().toISOString()
        };

        // Save using MOCK_DATA_API
        if (window.MOCK_DATA_API) {
            window.MOCK_DATA_API.saveCatchment(catchmentObj);
            console.log('✅ Catchment Request saved locally and Auto-Approved');

            // Show Success UI
            showSuccessMessage({
                success: true,
                acm_id: catchmentObj.catchment_id
            }, districtSelect, mandalSelect, panchayatSelect, villageSelect, cropSelect, seasonSelect, periodSelect);

            // Reset form
            document.getElementById('catchmentRequestForm').reset();
            if (typeof resetAllDropdowns === 'function') resetAllDropdowns();

        } else {
            throw new Error("MOCK_DATA_API not initialized");
        }

    } catch (error) {
        console.error('Error submitting request:', error);
        showError('An error occurred while saving the request locally.');
    } finally {
        // Restore button
...
function showSuccessMessage(data, districtSelect, mandalSelect, panchayatSelect, villageSelect, cropSelect, seasonSelect, periodSelect) {
    ...
    const summary = `
        ...
        <div style="background: #f0fff4; padding: 10px; border-radius: 4px; margin-top: 10px;">
            ...
            <strong>Status:</strong> <span style="color: #28a745; font-weight: bold;">Auto-Approved (Mock)</span>
        </div>
    `;
